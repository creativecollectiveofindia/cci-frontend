...
export class SubmissionFormComponent implements OnInit {
  ...
  challengeDetail: any = {'title' : '', 'introduction' : '', 'description' : '', 'challenge_goal' : ''};
  challengeId = '';
  ...
  let formValue = this.submissionForm.value;
    ...
    {
      this.reasonsErrorMessage = "Please select reason to withdraw the submission.";
    ...
    if (formValue.status == 'submitted' || formValue.status == 'draft')
    {
      ...
      {
        this.targetAudienceErrorMessage = "Please select profession/occupation.";
        var errorDiv = document.getElementById("targetAudienceCheckDiv");
        errorDiv.scrollIntoView();

        return;
      }
      else if (formValue.target_audience_other == '' && this.selectedTG.includes('other'))
      {
        this.targetAudienceErrorMessage = "Please provide other profession/occupation.";
        var errorDiv = document.getElementById("targetAudienceCheckDiv");
        errorDiv.scrollIntoView();
      ...
      if (formValue.special_condition == 'other' && formValue.special_condition_other == '')
      {
        this.specialAudienceErrorMessage = "Please provide other special condition.";
      ...
      if (this.checkedValuesLanguages.length == 0 || this.checkedValuesLanguages.length > 2)
      {
      ...
      else if (this.checkedValuesLanguages.indexOf('other') != -1 && formValue.language_other == '')
      {
    ...
    if (formValue.id != undefined && formValue.id != 0)
    {
      ...
      this.CollectionService.updateItem('submission', submissionId, formValue).subscribe(result => {
        ...
        else if (result.data.id != undefined && result.data.id != "" && result.data.id > 0)
        {
          for (var fileIdIndex in this.submissionFiles)
          {
            let submissionFile = {submission_id: result.data.id, directus_files_id: this.submissionFiles[fileIdIndex]}

            this.CollectionService.createItem('submission_directus_files', submissionFile).subscribe(result => {
              this.savedSubmissionFiles.push(result.data.id);
              let savedFileJunctionData = {junction_id: result.data.id, file_id: this.submissionFiles[fileIdIndex]};
      ...
      (error) => {
        document.documentElement.scrollTop = 0;
        this.saveErrorMessage = 'Something went wrong, Please try again later.';
      });
    }
    else
    {
      ...
      this.CollectionService.createItem('submission', formValue).subscribe(result => {
        this.submissionStatus = result.data.status;
        ...
        if (result.data.id != undefined && result.data.id != "" && result.data.id > 0)
        {
          for (var fileIdIndex in this.submissionFiles)
          {
            ...
            this.CollectionService.createItem('submission_directus_files', submissionFile).subscribe(fileResult => {
              this.savedSubmissionFiles.push(fileResult.data.id);
              let savedFileJunctionData = {junction_id: fileResult.data.id, file_id: this.submissionFiles[fileIdIndex]};
              this.savedSubmissionFilesJunctions.push(savedFileJunctionData);
              this.submissionFiles.splice(parseInt(fileIdIndex), 1);
            });
          }
  ...
  onFileSelect(event) {
    if (this.submissionStatus != 'draft'){
      this.saveErrorMessage = "You can not update your submission once it is submitted.";
      document.documentElement.scrollTop = 0;
      return;
    }
    ...
    if (event.target.files.length > 0) {
      ...
      let fileData = {name:file.name, size: file.size, fileId:'', progress:{}};
  ...
  deleteFile(file) {
    if (this.submissionStatus != 'draft'){
      this.saveErrorMessage = "You can't update your submission once it is submitted.";
      document.documentElement.scrollTop = 0;
      return;
    }
    ...
    this.CollectionService.deleteFile(file.fileId).subscribe(result => {
      ...
      {
        if (this.savedSubmissionFilesJunctions[fileJunctionMapIndex].file_id == file.fileId)
        {
          let fileJunctionId = this.savedSubmissionFilesJunctions[fileJunctionMapIndex].junction_id;

          this.CollectionService.deleteItem('submission_directus_files', fileJunctionId).subscribe(data => {
            if (this.savedSubmissionFilesJunctions[fileJunctionMapIndex] != undefined)
            {
              this.savedSubmissionFilesJunctions.splice(parseInt(fileJunctionMapIndex), 1);
            }
          });
        }
      }
      ...
      {
        if (this.savedSubmissionFiles[savedFileIndex].id != undefined && this.savedSubmissionFiles[savedFileIndex].id == file.fileId)
        {
  ...
  getSubmission() {
    this.CollectionService.getItem('submission', this.submissionId).subscribe(data => {
      ...
      this.submissionForm = this.formBuilder.group({
        ...
        title: [this.submission ? this.submission.title : '', [Validators.required, Validators.maxLength(120)]],
        ...
        challenge_category: [this.submission ? this.submission.challenge_category : '', Validators.required],
        challenge: [this.submission ? this.submission.challenge : '', Validators.required],
        file: [this.submission ? this.submission.file : ''],
        hidden_file: [''],
        language: [this.submission ? this.submission.language : ''],
        target_audience: [this.submission ? this.submission.target_audience : ''],
        geography: [this.submission ? this.submission.geography : '', Validators.required],
        region: [this.submission ? this.submission.region : '', Validators.required],
        income_bracket: [this.submission ? this.submission.income_bracket : '', Validators.required],
        gender: [this.submission ? this.submission.gender : '', Validators.required],
        age_group: [this.submission ? this.submission.age_group : '', Validators.required],
        educational_background: [this.submission ? this.submission.educational_background : ''],
        sector: [this.submission ? this.submission.sector : ''],
        special_condition: [this.submission ? this.submission.special_condition : ''],
        media_type: [this.submission ? this.submission.media_type : '', Validators.required],
        gender_other: [this.submission ? this.submission.gender_other : ''],
        sector_other: [this.submission ? this.submission.sector_other : ''],
        target_audience_other: [this.submission ? this.submission.target_audience_other : ''],
        special_condition_other: [this.submission ? this.submission.special_condition_other : ''],
        language_other: [this.submission ? this.submission.language_other : ''],
        withdraw_reasons: [this.submission ? this.submission.withdraw_reasons : ''],
      });

      this.CollectionService.getItems('creative_challenge', '?filter[category][eq]='+this.submission.challenge_category).subscribe(challengeResult => {
        this.topics = challengeResult.data;
      });

      let challengeId = <unknown> this.submission.challenge;
      this.CollectionService.getItem('creative_challenge', <string>challengeId).subscribe(data => {
        this.challengeDetail = data.data;
      });
    });

    this.CollectionService.getItems('submission_directus_files', `?filter[submission_id][eq]=${this.submissionId}&fields=*.*`).subscribe(data => {
        for (var fileData in data.data)
        {
          let savedFileData = {size: data.data[fileData].directus_files_id.filesize, name: data.data[fileData].directus_files_id.title, fileId: data.data[fileData].directus_files_id.id};
          this.savedSubmissionFiles.push(data.data[fileData].directus_files_id);
          let savedFileJunctionData = {junction_id: data.data[fileData].id, file_id: data.data[fileData].directus_files_id.id};
          this.savedSubmissionFilesJunctions.push(savedFileJunctionData);
          this.files.push(savedFileData);
        }

        if (this.submissionStatus == 'submitted' || this.submissionStatus == 'published' || this.submissionStatus == 'withdrawn')
        {
  ...
  getRegions() {
    this.CollectionService.getField('submission', 'region').subscribe(result => {
      let regionOptions = [];
      for (var region in result.data.options.choices) { 
        let regionObj = {value: region, label: result.data.options.choices[region]};
  ...
  getTargetAudience() {
    this.CollectionService.getField('submission', 'target_audience').subscribe(result => {
      let targetAudienceOptions = [];
      for (var targetAudience in result.data.options.choices) { 
        let targetAudienceObj = {value: targetAudience, label: result.data.options.choices[targetAudience]};
  ...
  getGeography() {
    this.CollectionService.getField('submission', 'geography').subscribe(result => {
      let geographyOptions = [];
      for (var geography in result.data.options.choices) { 
        let geographyObj = {value: geography, label: result.data.options.choices[geography]};
  ...
  getLanguageOptions() {
    this.CollectionService.getField('submission', 'language').subscribe(result => {
      let langOptions = [];
      for (var lang in result.data.options.choices) { 
        let langObj = {value: lang, label: result.data.options.choices[lang]};
  ...
  getIncomeBracket() {
    this.CollectionService.getField('submission', 'income_bracket').subscribe(result => {
      let incomeOptions = [];
      for (var income in result.data.options.choices) { 
        let langObj = {value: income, label: result.data.options.choices[income]};
  ...
  getGender() {
    this.CollectionService.getField('submission', 'gender').subscribe(result => {
      let genderOptions = [];
      for (var gender in result.data.options.choices) { 
        let genderObj = {value: gender, label: result.data.options.choices[gender]};
  ...
  getAgeGroup() {
    this.CollectionService.getField('submission', 'age_group').subscribe(result => {
      let ageOptions = [];
  ...
  getEducationalBackground() {
    this.CollectionService.getField('submission', 'educational_background').subscribe(result => {
      let eduOptions = [];
  ...
  getSector() {
    this.CollectionService.getField('submission', 'sector').subscribe(result => {
      let sectorOptions = [];
      for (var sec in result.data.options.choices) { 
        let sectorObj = {value: sec, label: result.data.options.choices[sec]};
  ...
  getSpecialCondition() {
    this.CollectionService.getField('submission', 'special_condition').subscribe(result => {
      let specialConditionOptions = [];
      for (var specialCondition in result.data.options.choices) 
      { 
        let specialConditionObj = {value: specialCondition, label: result.data.options.choices[specialCondition]};
  ...
  onCheckTargetAudience(event) 
  {
    ...
    {
      const index = this.checkedValuesTargetAudience.indexOf(event.target.value);
  ...
  {
    if (this.checkedValuesLanguages.length > 1 && event.target.checked == true)
    {
  ...
  onChanges(event)
  {
    this.CollectionService.getItem('creative_challenge', event.target.value).subscribe(data => {
        this.challengeDetail = data.data;
    });
  }

  getCategories() {
    this.CollectionService.getItems('challenge_category').subscribe(result => 
    {
  ...
  getChallenges(event)
  {
    this.CollectionService.getItems('creative_challenge', '?filter[category][eq]='+event.target.value).subscribe(challengeResult => {
      this.topics = challengeResult.data;
    });
  }

  getMediaType() {
    this.CollectionService.getField('submission', 'media_type').subscribe(result => {
      let mediaOptions = [];
      for (var media in result.data.options.choices) { 
        let mediaObj = {value: media, label: result.data.options.choices[media]};
  ...
  deleteSubmission()
  {
    this.loading = true;
    let formValue = this.submissionForm.value;
    this.CollectionService.deleteItem('submission', formValue.id).subscribe(data => {
      this.router.navigate(['/submission-list/']);
    });
  }
  ...
  getWithdrawReasons()
  {
    this.CollectionService.getField('submission', 'withdraw_reasons').subscribe(result => {
      let reasonsOptions = [];
      for (var reasons in result.data.options.choices) { 
        let reasonsObj = {value: reasons, label: result.data.options.choices[reasons]};
